...
public class ModifyCustomerViewController implements Initializable {
     ...
     * Checks include, but are not limited to: phone number is valid, nameField is not numeric, all textFields are filled in.
    ...
    public void saveMod(ActionEvent event) throws IOException {
        ...
        if ( Util.isNumeric(nameField.getText()) || Util.isNumeric(addressField.getText())) {
            caseInt = 1;
        } else if (nameField.getText() == null || addressField.getText() == null || phoneField.getText() == null || postalField.getText() == null) {
            caseInt = 2;
        ...
        switch (caseInt) {
            case 1:
                ...
                break;
            case 2:
                Alert filledAlert = new Alert(Alert.AlertType.ERROR);
                filledAlert.setTitle("Missing Fields");
                filledAlert.setHeaderText("Please fill in all of the fields");
                filledAlert.show();
                break;
            case 3:
            ...
            case 6:
                try {
                    int index = displayedCustomer.getID();
                    ...
                    Customer.updateCustomerInDB(displayedCustomer.getID(), updatedCustomer);
    ...
    /** Initializes Modify Customer screen. Sets titles, and items inside combo boxes */
    @Override
    public void initialize(URL url, ResourceBundle resourceBundle) {

        countryCombo.setValue(Util.getCountry(displayedCustomer.getDivisionName()));
        divisionCombo.setValue(displayedCustomer.getDivisionName());

        setupCountryCombo();
        setupDivisionCombo();

        countryCombo.setOnAction(e -> {
            System.out.println("Country Combo Action");
            setupDivisionCombo();
        });


        customerID.setText(String.valueOf(displayedCustomer.getID()));
        nameField.setText(String.valueOf(displayedCustomer.getName()));
        phoneField.setText(displayedCustomer.getPhoneNumber());
        postalField.setText(String.valueOf(displayedCustomer.getPostalCode()));
        addressField.setText(String.valueOf((displayedCustomer.getAddress())));
